...
- Enable cross-document synthesis and personalisation.
- Support scalable deployment for institutions and enterprises.
## Modules of the System
The system is structured into several core modules, each tasked with a distinct stage in the document understanding and response generation pipeline. These modules work in coordination to deliver accurate, personalised, and multimodal outputs.
### Document Ingestion
...
3. Organising documents into a consistent internal structure for downstream processing.
...
After retrieving the relevant content, this module uses generative AI models (like GPT) to synthesise responses in various formats:
...
5. **Response Generation:** Use generative AI models to synthesise coherent, informative, and multimodal responses.
...
Furthermore, traditional document summarisation tools are generally single-format focused and cannot process and integrate information across multiple types of documents such as PDFs, spreadsheets, and web pages.
### Limitations in Existing Systems
- No support for cross-format knowledge synthesis.
- Lack of personalised or dynamic output generation.
...
The platform offers personalised output generation through role-based customisation, and it ensures scalability via cloud deployment. Unlike existing tools, it leverages vector databases, LangChain orchestration, and transformer-based NLP models to provide a robust end-to-end solution.
### Feasibility Study
...
- Open-source libraries and frameworks minimise initial development and licensing costs.
...
- Built using Gradio, it supports interactive document uploads, role-based personalisation, and easy navigation.
...
- **Podcasts:** Summarised content converted to speech using text-to-speech APIs.
...
- Simultaneous processing of at least 20 concurrent users shall be supported without degradation in performance on the standard Azure VM configuration.
### Scalability
...
### Portability
- The application shall run in containerised environments and be deployable on any cloud platform (e.g., Azure, AWS, GCP) or on-premise system with Docker support.
...
- Role-based access control (RBAC) shall be enforced to prevent unauthorised access to administrative functions.
...
- The UI text shall be translatable based on user preference, supporting basic localisation features.
...
The platform is especially useful for academic institutions automating course material creation, professionals compiling reports and training modules, and organisations managing large volumes of informational content. Overall, the SRS presents the AI-Driven Knowledge Synthesis Platform as a forward-thinking, AI-powered solution that bridges the gap between unstructured knowledge sources and personalised, actionable insights.
...
- **Internationalisation:** Supports English and Hindi localisation.
## Use Case Mode
...
As the number of processed documents and corresponding vector embeddings increases, the performance of the FAISS vector database may degrade without the implementation of techniques like sharding or hierarchical indexing. In its current form, the system is optimised for moderate-scale use but may require architectural upgrades to handle enterprise-level data volumes.
## Future Enhancements
...
Incorporating fine-tuned versions of large language models trained on specific domains (e.g., legal, medical, or academic texts) can significantly improve the accuracy and contextual relevance of the synthesised outputs. This will also allow the system to adapt better to industry-specific terminology.
